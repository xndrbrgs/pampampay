...
export default async function DashboardLayout({
  ...
  const stripeConnectedLinked = await getStripeConnnectUser(userId);
  const stripeConnected = stripeConnectedLinked?.stripeConnectedLinked

  return (
    <main className="relative w-full">
      <div className="flex h-full">
        <div className="hidden md:block sticky top-0 left-0">
          <Sidebar stripeConnected={stripeConnected} />
        </div>
          ...
          <div className="sticky top-0 z-50 backdrop-blur-xl shadow-lg">
            <div className="md:hidden flex justify-between items-center p-4">
              <div className="relative flex items-center">
                ...
                <span className="font-editorial text-lg pt-1 pl-2">
                  PamPamPay
                </span>
              </div>
              <MobileNav stripeConnected={stripeConnected} />
            </div>
          </div>
