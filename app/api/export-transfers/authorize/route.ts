...
export async function GET(request: Request) {
  ...
  const transfers = await prisma.coinbaseTransfer.findMany({
    where: {
      createdAt: {
        gte: start,
        lte: end,
  ...
  const stripeFeePercentage = 1; // Stripe fee percentage (2.9%)
  const stripeFlatFee = 0; // Stripe flat fee ($0.30 per transaction)
  ...
  const fileName = `AUTransfers.xlsx`;
