...
export async function GET(request: Request) {
  ...
  const transfers = await prisma.squareTransfer.findMany({
    where: {
      createdAt: {
        gte: start,
        lte: end,
  ...
  const squareFeePercentage = 0.029; // Square fee percentage (2.9%)
  const squareFlatFee = 0.3; // Square flat fee ($0.30 per transaction)

  const netAmount = data.reduce((sum, transfer) => {
    const fee = transfer.Amount * squareFeePercentage + squareFlatFee;
  ...
  const fileName = `SQTransfers.xlsx`;
