...
import { isGooglePayAvailable, processDigitalWalletPayment } from "@/services/authdigitalwallets"
...
export function GooglePayButton({
  ...
  useEffect(() => {
    const checkAvailability = async () => {
      ...
      if (available && window.google && window.google.payments && window.google.payments.api) {
  ...
  const handleGooglePayment = async () => {
    ...
    debug += `API Login ID: ${process.env.NEXT_PUBLIC_AUTHORIZE_NET_API_LOGIN_ID ? "Present (masked)" : "Missing"}\n`
    debug += `Merchant ID: ${process.env.NEXT_PUBLIC_GOOGLE_PAY_MERCHANT_ID ? "Present (masked)" : "Missing"}\n`
    ...
    try {
      ...
      debug += `Payment data request: ${JSON.stringify(paymentDataRequest, null, 2)}\n`
      setDebugInfo(debug)
      ...
      debug += `Payment data received: ${JSON.stringify(paymentData, null, 2)}\n`
      setDebugInfo(debug)
